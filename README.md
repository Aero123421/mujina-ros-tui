...
- Real Launch: Dashboard のFlow一覧から開きます。`n` / `u` でCAN mode、`1` / `2` / `3` でoperator checklist、必要なら `4` / `F4` で起動姿勢一致gateだけをskip、`REAL` 入力後に `Enter` / `Ctrl+E` で段階起動します。P0/P1/P2 lockや競合jobが残っている場合は起動しません。
