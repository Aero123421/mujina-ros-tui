...
from mujina_assist.services import can as can_service
from mujina_assist.services import devices as device_service
...
def workspace_signature(paths: AppPaths) -> str:
    return upstream.workspace_signature(paths)


def detect_real_devices() -> dict[str, bool]:
    return device_service.detect_real_devices()


def list_serial_device_candidates() -> list[str]:
    return device_service.list_serial_device_candidates()
...
def workspace_clone_ready(paths: AppPaths) -> bool:
    return upstream.workspace_exists(paths)
...
def write_config_file(paths: AppPaths) -> None:
    ...
    payload = {
        ...
        "third_party_dir": str(paths.third_party_dir),
        "vendored_upstream_dir": str(paths.vendored_upstream_dir),
        "patches_dir": str(paths.patches_dir),
        "upstream_patches_dir": str(paths.upstream_patches_dir),
    }
...
def inspect_can_status() -> dict[str, object]:
    return can_service.inspect_can_status()
