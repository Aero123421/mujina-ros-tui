...
def activate_policy(paths: AppPaths, state: RuntimeState, candidate: PolicyCandidate, log_path: Path) -> tuple[bool, str]:
    ...
    if candidate.manifest_path and candidate.manifest_path.exists():
        manifest_validation = validate_policy_manifest(candidate.manifest_path, policy_path=candidate.path)
        if not manifest_validation.ok:
            return False, "policy manifest が不正です: " + " / ".join(manifest_validation.errors)
