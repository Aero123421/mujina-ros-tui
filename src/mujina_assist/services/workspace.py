...
from mujina_assist.services.upstream import (
    ASSISTED_MODE,
    clone_to_vendored_command,
    prepare_workspace_command_result,
    vendored_upstream_exists,
)
...
def ensure_upstream_clone(paths: AppPaths, log_path: Path) -> CommandResult:
    ...
    if workspace_clone_ready(paths):
        return CommandResult(command="prepare vendored mujina_ros", returncode=0, stdout="already prepared")
    if not vendored_upstream_exists(paths):
        if paths.vendored_upstream_dir.exists() and any(paths.vendored_upstream_dir.iterdir()):
            shutil.rmtree(paths.vendored_upstream_dir, ignore_errors=True)
        paths.third_party_dir.mkdir(parents=True, exist_ok=True)
        clone_result = run_bash(
            clone_to_vendored_command(UPSTREAM_REPO, paths),
            cwd=paths.repo_root,
            log_path=log_path,
            interactive=True,
        )
        if clone_result.returncode != 0:
            return clone_result
    return prepare_workspace_command_result(
        paths,
        log_path,
        mode=ASSISTED_MODE,
        replace=True,
