...
class MujinaAssistApp:
    ...
    def _current_workspace_signature(self) -> str:
        before = (
            self.state.workspace_signature,
            self.state.last_sim_success,
            self.state.last_sim_verified_workspace_signature,
        )
        sync_runtime_workspace_state(self.paths, self.state)
        after = (
            self.state.workspace_signature,
            self.state.last_sim_success,
            self.state.last_sim_verified_workspace_signature,
        ...
        if before != after:
            self.save_state()
        return self.state.workspace_signature or workspace_signature(self.paths)
    ...
    def handle_real_robot(self, can_mode: str = "auto", *, skip_startup_pose_gate: bool = False) -> int:
        title("実機起動")
        self._sync_relogin_requirement()
        if not self._require_built_workspace():
            return 1
        ...
        if not self._prepare_real_can_link(selected_can_mode):
            return 1
        if skip_startup_pose_gate:
            warn("起動姿勢一致gateをスキップします。12軸の応答・速度・電流・温度・error code確認は実行します。")
        if not self._run_real_motor_preflight_scan(selected_can_mode, skip_startup_pose_gate=skip_startup_pose_gate):
            return 1
        ...
        safety = evaluate_real_preflight(
            ...
            zero_profile=self._active_zero_profile_validation(),
            can_mode=selected_can_mode,
            active_job_kinds={job.kind for job in active_jobs(self.paths)},
            operator_checklist_complete=True,
            real_confirmation=typed,
        )
        hard_blocks = p0_reasons(safety)
        if hard_blocks:
    ...
    def _run_real_motor_preflight_scan(self, can_mode: str, *, skip_startup_pose_gate: bool = False) -> bool:
        ...
        errors = validate_scan_for_real_launch(scan_result, require_safe_pose=not skip_startup_pose_gate)
        if errors:
        ...
        bullet(f"scan: {scan_path}")
        if skip_startup_pose_gate:
            warn("姿勢一致gateは明示的にスキップされています。起動姿勢はoperator checklistで確認してください。")
        return True
    ...
    def _execute_real_launch_job(self, job: JobRecord) -> tuple[int, str, bool]:
        ...
        skip_startup_pose_gate = bool(job.payload.get("skip_startup_pose_gate", False))
        ...
        if skip_startup_pose_gate:
            warn("起動姿勢一致gateをスキップします。12軸の応答・速度・電流・温度・error code確認は実行します。")
        if not self._run_real_motor_preflight_scan(can_mode, skip_startup_pose_gate=skip_startup_pose_gate):
    ...
    def _active_policy_manifest_validation(self):
        candidate = self._current_active_policy_candidate()
        if candidate is None or candidate.source_type == "default":
            return None
        if candidate.manifest_path is None or not candidate.manifest_path.exists():
            return None
...
def build_parser() -> argparse.ArgumentParser:
    ...
    menu_parser.add_argument("--legacy", action="store_true", help="旧番号メニューを起動します")
    subparsers.add_parser("legacy-menu")
    ...
    robot_parser.add_argument("--skip-startup-pose-gate", action="store_true")
...
def run_app(repo_root: Path, argv: list[str] | None = None) -> int:
    ...
    if command == "menu":
        if args.legacy:
            return app.run_menu()
        from mujina_assist.tui import run_tui

        return run_tui(repo_root)
    if command == "legacy-menu":
        return app.run_menu()
    if command == "tui":
        from mujina_assist.tui import run_tui
    ...
    if command == "robot":
        return app.handle_real_robot(can_mode=args.can_mode, skip_startup_pose_gate=args.skip_startup_pose_gate)
    if command == "policy":
